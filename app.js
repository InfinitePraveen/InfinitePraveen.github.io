...
function researchCard(r){
 ...
 const articleUrl=`research/${r.id}.html`;
...
fetch('data.json').then(r=>r.json()).then(data=>{D=data;setupFields();renderProjects();renderResearch();renderPresentations();}).catch(()=>{document.querySelector('main').insertAdjacentHTML('afterbegin','<div class="empty">Unable to load portfolio data. Please serve the site from GitHub Pages or a local web server.</div>');});
